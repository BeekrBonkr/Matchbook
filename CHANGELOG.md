...
**Fixed: a recurring "empty team, stats carried over from the last time the map was played" bug. This is a different bug than the one already fixed in 0.6.9/0.6.10 with the same symptom.**
...
- The admin/browse-all matches list reused the same item-lore builder as the personal match history view, so every match item showed *your* Kills/Final Kills/Final Deaths/Beds, which is meaningless (usually all zero) for matches you never played in. The All Matches view no longer includes any per-viewer stat lines; your personal match history is unaffected.
